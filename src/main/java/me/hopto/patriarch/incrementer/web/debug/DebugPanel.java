...
/**
 * my quest for a generic debug bar is just a giant pain in the ass after all..
 * Can't re-render only part of my repeater without going to astronomic length
 * and rewriting the whole ListView class. Is it worth it ? i'll just take the
 * best of both worlds. Static html / css, dynamic Java. And come back to this
 * task after having done some significant improvements on the game."
 */
...
public class DebugPanel extends Panel {
		...
		add(new AbstractAjaxTimerBehavior(Duration.milliseconds(200d)) {
			...
			protected void onTimer(AjaxRequestTarget target) {
				...
				if (params.getObject().getAutoClickParam().getValue()) {
					for (int i = 0; i < params.getObject().getFoodParam()
							.getValue(); i++)
						built.getObject().incrementOne(ResourceType.Food);
					for (int i = 0; i < params.getObject().getWoodParam()
							.getValue(); i++)
						built.getObject().incrementOne(ResourceType.Wood);
					for (int i = 0; i < params.getObject().getMetalParam()
							.getValue(); i++)
						built.getObject().incrementOne(ResourceType.Metal);
					for (int i = 0; i < params.getObject().getToolParam()
							.getValue(); i++)
				...
				if (params.getObject().getAutoBuyParam().getValue()) {
					if (hasSlept % 600 == 400)
						built.getObject().levelUp(BuildingType.BlackSmith);
					if (hasSlept % 500 == 400)
						built.getObject().levelUp(BuildingType.Inventor);
					if (hasSlept % 600 == 400)
						built.getObject().levelUp(BuildingType.Miner);
					if (hasSlept % 500 == 400)
						built.getObject().levelUp(BuildingType.MetalDigger);
					if (hasSlept % 500 == 300) 
						built.getObject().levelUp(BuildingType.LumberJack);
					if (hasSlept % 500 == 100) 
						built.getObject().levelUp(BuildingType.WoodGatherer);
					if (hasSlept % 300 == 200) 
						built.getObject().levelUp(BuildingType.BerryPicker);
					if (hasSlept % 300 == 100) 
						built.getObject().levelUp(BuildingType.FisherMan);
				}
			}
		});
	}
	...
	private void build(DebugList params) {
		WebMarkupContainer actionRow = new WebMarkupContainer(
				"resourcerowAction");
		actionRow.add(AttributeModifier.replace("class", "resourcerow"));

		WebMarkupContainer reourcesTable = new WebMarkupContainer(
				"resourcesTable");
		...
		for (final DebugParam param : params) {
			...
			Label value = new Label("action" + title,
					new DebugParamModel(param));
